...
import type { FlowRecord, FlowStatus, FlowView } from "@/lib/atlas/flow-map-store";
...
const flowViews: { id: FlowView; label: string; helper: string }[] = [
  ...
  { id: "atlas", label: "Turicum LLC", helper: "Underwriting, legal, funding, and control" },
...
const laneOrder = ["Atlas", "Borrower", "Investor"] as const;
const laneMeta = {
  Atlas: { helper: "Control, validation, legal, funding", tone: "atlas" },
...
const canonicalLaneById = new Map<string, LaneName>([
  ...
  ["borrower-property-validation", "Atlas"],
  ["investor-promotion", "Investor"],
  ["investor-workspace", "Investor"],
  ["template-selection", "Atlas"],
  ["contract-ai-review", "Atlas"],
  ["legal-review", "Atlas"],
  ["closing-diligence", "Atlas"],
...
function nodeClassForStatus(status: FlowStatus, muted: boolean) {
  return `atlas-flow-node atlas-flow-node-${status}${muted ? " atlas-flow-node-muted" : ""}`;
}
...
function buildNodes(records: FlowRecord[], activeView: FlowView, presentationMode: boolean): Node[] {
  return sortRecords(records).map((record, index) => {
    ...
    return {
      ...
      data: {
        label: (
          <div className={`atlas-flow-card atlas-flow-card-compact atlas-flow-card-${lane.toLowerCase()}${presentationMode ? " atlas-flow-card-presentation" : ""}`}>
            <div className="atlas-flow-card-top atlas-flow-card-top-tight">
              <span className="atlas-flow-step-index">Step {index + 1}</span>
              {!presentationMode ? <span className={`atlas-flow-status atlas-flow-status-${record.status}`}>{record.status}</span> : null}
            </div>
            <strong>{record.title}</strong>
            <p>{record.summary}</p>
            <div className="atlas-flow-meta-row">
              <span className="atlas-flow-chip atlas-flow-chip-stage">{stage?.label ?? "Flow"}</span>
              <span className={`atlas-flow-chip atlas-flow-chip-lane atlas-flow-chip-lane-${lane.toLowerCase()}`}>{displayLane(lane)}</span>
              {!presentationMode ? <span className="atlas-flow-chip atlas-flow-chip-muted">{record.phase}</span> : null}
            </div>
          </div>
        )
      },
...
function buildEdges(edges: Edge[], visibleIds: Set<string>) {
  return edges
    ...
    .map((edge) => {
      const targetLane = canonicalLaneById.get(edge.target) ?? canonicalLaneById.get(edge.source) ?? "Atlas";
...
export function FlowMap({
  ...
  return (
    <section className={`panel atlas-flow-shell${presentationMode ? " atlas-flow-shell-presentation" : ""}`}>
      <div className="section-head atlas-flow-topbar">
        <div>
          ...
          <p className="helper atlas-flow-topbar-copy">
            {presentationMode
        ...
        <div className="form-actions">
          <div className="atlas-flow-mode-toggle" role="tablist" aria-label="Flow modes">
      ...
      {!presentationMode ? (
        <>
          <div className="atlas-flow-summary-band">
          ...
          <div className="atlas-flow-toolbar">
            <div className="atlas-flow-view-tabs" role="tablist" aria-label="Flow views">
              {flowViews.map((view) => (
                <button
                  key={view.id}
                  type="button"
                  className={`atlas-flow-view-tab${activeView === view.id ? " atlas-flow-view-tab-active" : ""}`}
            ...
            <label className="field atlas-flow-search">
      ...
      <div className={`atlas-flow-layout${selectedRecord && !presentationMode ? " atlas-flow-layout-with-inspector" : ""}`}>
        <div className="atlas-flow-canvas-wrap">
          {presentationMode ? (
            <div className="atlas-flow-overview-rail" aria-hidden="true">
              {visibleStages.map((stage, index) => {
                const summary = stageSummaries.find((item) => item.key === stage.key);
                return (
                  <div key={stage.key} className={`atlas-flow-overview-stage-card atlas-flow-overview-stage-card-${stage.tone}`}>
                    <div className="atlas-flow-overview-stage-top">
                      <span className="atlas-flow-overview-stage-index">{String(index + 1).padStart(2, "0")}</span>
                      <span className="atlas-flow-overview-stage-badge">{summary?.steps ?? 0} steps</span>
                    </div>
                    <strong>{stage.label}</strong>
                    <span>{stage.helper}</span>
                    <div className="atlas-flow-overview-stage-meta">
          ...
          <div className="atlas-flow-canvas">
            <div className="atlas-flow-stages" aria-hidden="true">
              {visibleStages.map((stage, index) => (
                <div key={stage.key} className={`atlas-flow-stage atlas-flow-stage-${stage.tone}`} style={{ left: stage.x, width: stage.width }}>
                  <span className="atlas-flow-stage-index">{String(index + 1).padStart(2, "0")}</span>
                  <span className="atlas-flow-stage-label">{stage.label}</span>
                  <span className="atlas-flow-stage-helper">{stage.helper}</span>
                </div>
              ))}
            </div>
            <div className="atlas-flow-swimlanes" aria-hidden="true">
              {laneOrder.map((lane) => (
                <div key={lane} className={`atlas-flow-swimlane atlas-flow-swimlane-${lane.toLowerCase()}`}>
                  <span className="atlas-flow-swimlane-label">{lane}</span>
                  <span className="atlas-flow-swimlane-helper">{laneMeta[lane].helper}</span>
                </div>
              ))}
            </div>
            <ReactFlow
            ...
            >
              ...
              <Panel position="top-left" className="atlas-flow-legend atlas-flow-legend-strong">
          ...
          {!presentationMode ? (
            <div className="atlas-flow-register">
              ...
              <div className="atlas-flow-register-list">
                {filteredRecords.map((record, index) => (
                  <button key={record.id} type="button" className={`atlas-flow-register-item${selectedRecord?.id === record.id ? " atlas-flow-register-item-active" : ""}`} onClick={() => selectRecord(record.id)}>
                    <div className="atlas-flow-register-top">
                      <span className="atlas-flow-register-index">{index + 1}</span>
                      <strong>{record.title}</strong>
                      <span className={`atlas-flow-status atlas-flow-status-${record.status}`}>{record.status}</span>
                    </div>
                    <p>{record.summary}</p>
                    <div className="atlas-flow-meta-row">
                      <span className="atlas-flow-chip atlas-flow-chip-stage">{stageMetaById.get(record.id)?.label ?? "Flow"}</span>
                      <span className={`atlas-flow-chip atlas-flow-chip-lane atlas-flow-chip-lane-${canonicalLane(record).toLowerCase()}`}>{canonicalLane(record)}</span>
        ...
        {selectedRecord && !presentationMode ? (
          <aside className="atlas-flow-inspector">
            <div className="subpanel atlas-flow-inspector-panel">
              <div className="stack-sm">
                <p className="eyebrow">Selected step</p>
                <h3>{selectedRecord.title}</h3>
                <div className="atlas-flow-meta-row">
                  <span className="atlas-flow-chip atlas-flow-chip-stage">{stageMetaById.get(selectedRecord.id)?.label ?? "Flow"}</span>
                  <span className={`atlas-flow-chip atlas-flow-chip-lane atlas-flow-chip-lane-${canonicalLane(selectedRecord).toLowerCase()}`}>{canonicalLane(selectedRecord)}</span>
                  <span className="atlas-flow-chip atlas-flow-chip-muted">{selectedRecord.phase}</span>
                  <span className={`atlas-flow-status atlas-flow-status-${selectedRecord.status}`}>{selectedRecord.status}</span>
                </div>
              </div>

              <div className="atlas-flow-detail-grid">
              ...
              {downstreamRecords.length ? (
                <div className="callout">
                  ...
                  <div className="atlas-flow-related-grid">
                    {downstreamRecords.map((record) => (
                      <button key={record.id} type="button" className="atlas-flow-related-card" onClick={() => selectRecord(record.id)}>
                        <strong>{record.title}</strong>
                        <p>{record.summary}</p>
                      </button>
                    ))}
