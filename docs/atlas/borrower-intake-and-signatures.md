...
Atlas should handle two distinct layers:

1. `Borrower intake`
   Collect information directly from the borrower or guarantor in a controlled portal.
2. `Formal signature routing`
   Send signature-bearing documents through a dedicated provider after Atlas reviews the intake.
...
5. Borrower completes the packet in Atlas.
...
The borrower may complete form fields in Atlas, but a legal signature flow should still be routed through a signature provider for:
...
Atlas currently tracks signature requests and provider choice, but it does not yet call an external signature API.
