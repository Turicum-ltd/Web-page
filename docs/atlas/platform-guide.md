...
Turicum LLC is the operating layer for real-estate deal execution under `marketshift.net/atlas`.
...
- Background check
- Criminal check
- Whether the borrower will provide reports or Atlas will use an outside vendor

### Borrower intake workspace

Route:

- `/atlas/cases/[id]/intake`
...
This is the borrower-collection console for Atlas operations.

### Borrower portal

Route:

- `/atlas/borrower/[token]`
...
- `/atlas/cases/[id]/investor-promotion`
...
- `/atlas/library`
- `/atlas/library/templates`
- `/atlas/library/templates/[groupKey]`
...
Atlas now ranks newer documents ahead of older ones when the rest of the fit is comparable.

### Contract AI review

Route:

- `/atlas/cases/[id]/contract-ai-review`
...
- `/atlas/cases/[id]/closing-diligence`
...
Atlas now uses two legal-adjacent gates:
...
- Atlas ops for tracking and gate control
...
Atlas should reference:

- Drive file IDs
- Drive folder IDs
- Linked Drive URLs

Atlas should avoid becoming the primary document store unless there is a specific product reason.
...
- Atlas stores links, workflow state, and readiness

## Persistence model

### Current state

Atlas still supports local JSON fallback for rapid local operation.
...
Atlas is being migrated toward Supabase for workflow state while keeping Google Drive for documents.
...
This lets Atlas move review and lifecycle state out of local files without forcing a huge all-at-once migration.
...
- working files
- executed files
- external collaboration with non-Atlas participants
...
- PM2 or systemd
- reverse proxy
- Cloudflare Access in front of `/atlas`

Why:

- Atlas still has local fallback state
- Atlas has custom route/proxy behavior
- Atlas benefits from direct server control during iteration
