...
import type { DealIntakeAssessment } from "@/lib/turicum/deal-intake";
...
export function getClosingDiligenceDocumentBuckets(assessment: DealIntakeAssessment | null) {
  if (!assessment) {
    return {
      ...
      turicumPreparedDocuments: [] as Array<{ code: string; label: string; reason: string }>
    };
  }

  return {
    titleAgencyDocuments: assessment.contractStack.filter((item) => TITLE_AGENCY_DOCUMENT_CODES.has(item.code)),
    turicumPreparedDocuments: assessment.contractStack.filter((item) => !TITLE_AGENCY_DOCUMENT_CODES.has(item.code))
  };
}
